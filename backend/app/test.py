...
# 1. Raw Amplitude (with proper scaling)
axs[0].plot(freq, result.model_amp, 'r-', linewidth=2.2, label='Model')
axs[0].plot(freq, result.exp_amp, 'ko', markersize=4, label='Experiment')
...
# 2. Normalized Amplitude (shape comparison)
model_norm = result.model_amp / result.model_amp[0]
exp_norm = result.exp_amp / result.exp_amp[0]

axs[1].plot(freq_log, np.log10(model_norm), 'r-', linewidth=2.2, label='Model')
axs[1].plot(freq_log, np.log10(exp_norm), 'ko', markersize=4, label='Experiment')
...
# 3. Phase
axs[2].plot(freq_log, result.model_phase_deg, 'r-', linewidth=2.2, label='Model')
