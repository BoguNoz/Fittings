...
def simulations_ptr_hankel(
        ...
        *,
        ...
        alfa3: float = 6.0e-6,
        r21: float = 2.8e-8,
        d_pump: float = 2.42e-6,  # średnica wiązki pompy (1/e²)
        Q: float = 1.0,  # moc pompy (normalizacja)
) -> tuple[np.ndarray, np.ndarray]:
    """
    Poprawiona implementacja równania (4) z publikacji (Hankel transform).
    Zwraca (amplitude, complex_signal)
    """
    ...
    def integrand(lam: float, om: float) -> complex:
        ...
        # Współczynniki zgodnie z Twoim wcześniejszym 1D modelem
        ...
        # Poprawny czynnik Gaussa z równania (4)
        gaussian = np.exp(- (lam * d_pump) ** 2 / 8.0)

        return theta * gaussian * lam

    for i, om in enumerate(omega):
        # Całkujemy real i imag osobno dla lepszej stabilności
    ...
    # Prefaktory z równania (4)
    y_complex *= -Q / (2 * np.pi)

    # Kluczowy czynnik dyfuzyjny dla poprawnego zachowania fazy
