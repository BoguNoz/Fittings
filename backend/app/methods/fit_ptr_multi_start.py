...
def fit_ptr_multi_start(
        ...
        exp_amp: np.ndarray,
        exp_phase: np.ndarray,      # MUST be unwrapped already!
        n_starts: int = 35,
        **phys_params
) -> PTRFitResult:
    """
    Multi‑start PTR fitting with Latin Hypercube and physically motivated starts.
    Assumes experimental phase has been unwrapped (np.unwrap) before calling.
    """
    best_result = None
    best_resnorm = np.inf
    all_results = []
    ...
    # Bounds for optimization [log10(k2), log10(alfa2), log10(r32), log10(k3), phi0_deg]
    lower_bounds = [np.log10(0.02), np.log10(1e-9), np.log10(1e-10), np.log10(0.2), -180]
    upper_bounds = [np.log10(2.0),  np.log10(1e-5), np.log10(1e-5),  np.log10(8.0),  180]
    ...
    # Scale amplitude to match experiment (use first few points for gain)
    gain = np.mean(exp_amp[:5] / np.abs(y_model_norm[:5]))
    model_amp_scaled = np.abs(y_model_norm) * gain

    # Prepare experimental data for plotting (already normalized)
