...
def correct_ptr_data(
    ...
    """Unwrap phase and optionally remove amplitude outliers."""
    ...
    if remove_amp_outliers and len(amp) > 5:
        window = min(5, len(amp) // 2 * 2 + 1)
        amp_smooth = medfilt(amp, kernel_size=window) if window >= 3 else amp
        rel_diff = np.abs(amp - amp_smooth) / np.maximum(amp_smooth, 1e-12)
        mask = rel_diff < outlier_threshold
        if np.sum(mask) >= 5:
            freq = freq[mask]
            amp = amp[mask]
