...
def thermal_wavevector(k_perp: float, alpha_perp: float, k_in: float, lam: float, omega: float):
    """Wektor falowy wg. Eq. (9) dla TR (model 3D Henkela)"""
    return np.sqrt(1j * omega / alpha_perp + (k_in / k_perp) * lam**2 + 0j)

def layer_transfer_matrix(k: float, alpha: float, thickness: float, lam: float, omega: float, k_in=None):
    """Macierz przejścia warstwy anizotropowej (TR)"""
    if thickness < 1e-12:
...
def interface_matrix(R: float):
    """Macierz interfejsu z oporem cieplnym R"""
